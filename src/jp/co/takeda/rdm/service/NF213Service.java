 ...
 */
@Named
public class NF213Service extends BaseService {

    /**
     * ログインスタンス
     * @generated
     */
    private static Log log = LogFactory.getLog(NF213Service.class);
    ...
    /**
     * イベント処理
     * @param indto NF213DTO
     * @return 遷移先DTO
     * @customizable
     */
    @Transactional
    public BaseDTO init(NF213DTO indto) {
        ...
        if ("0".equals(indto.getDisplayKbn())) {
        	...
        	SelectNF213MainDataEntity paramEntity = new SelectNF213MainDataEntity("selectNF213InsTkdData");
        	...
        	// 施設固定コードで初期データ作成
        	// 武田紐領域別区分が'1'（領域別紐付け）
        	SelectNF213MainDataEntity paramEntity = new SelectNF213MainDataEntity("selectNF213InsTrtData");
    		paramEntity.setInInsNo(indto.getInsNo());
        	...
        	// 申請データ（一時保存含む）を参照
        	// 武田紐領域別区分が'0'（来期用紐付け(武田紐)）
        	SelectNF213MainDataEntity paramEntity = new SelectNF213MainDataEntity("selectNF213ReqTkdData");
     ...
     */
    @Transactional
    public BaseDTO cancel(NF213DTO indto) {
        BaseDTO outdto = indto;
     ...
     */
    @Transactional
    public BaseDTO register(NF213DTO indto) {
        BaseDTO outdto = indto;
        // START UOC
        LoginInfo loginInfo = (LoginInfo)BaseInfoHolder.getUserInfo();
        // 現在日付を取得
     ...
     */
    @Transactional
    public BaseDTO shnComp(NF213DTO indto) {
        BaseDTO outdto = indto;
        // START UOC
        LoginInfo loginInfo = (LoginInfo)BaseInfoHolder.getUserInfo();
        // 現在日付を取得
