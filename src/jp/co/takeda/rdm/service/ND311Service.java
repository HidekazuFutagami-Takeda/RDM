    ...
    public BaseDTO init(ND311DTO dto) {
      ...
      //初期表示
       ...
       //画面初期表示時の帳票一覧を取得する
    	...
    	paramStatus.setSqlId("selectByValue");
        //申請ステータスの帳票一覧を取得する
        ...
        for (SelectHcpKmuReqNewEntity entity : SelectHcpKmuReqNewList) {
     	 ...
     	 //医師メニュースイッチ
        	...
        	for (MRdmHcpYakusyokuEntity outEntity : SelectYakusyoku) {
        		if (Objects.equals(outEntity.getTitleCode(),entity.getTitleCodeAf())) {
        	...
        	for (MRdmCodeMstEntity outEntity : SelectKinmuKeitaiList) {
        		if (Objects.equals(outEntity.getValue1(),entity.getJobFormAf())) {
        			dto.setJobFormAfName(outEntity.getValue1Kanj());
        			break;
        		}
        	}
        	//薬審メンバー区分(異動先)
         for (MRdmCodeMstEntity outEntity : SelectYakushinList) {
         	if (Objects.equals(outEntity.getValue1(),entity.getDccTypeAf())) {
        			dto.setDccTypeAfName(outEntity.getValue1Kanj());
        			break;
        		}
         }
        	//大学職位コード(異動先)
         for (MRdmHcpYakusyokuEntity outEntity : SelectDaigakuSyokui) {
         	if (Objects.equals(outEntity.getTitleCode(),entity.getUnivPosCodeAf())) {
        			dto.setUnivPosCodeAfName(outEntity.getTitleKj());
        			break;
        		}
         }
        	//適用開始日
