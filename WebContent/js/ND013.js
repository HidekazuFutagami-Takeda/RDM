...
window.onload = function(){
	...
	if (document.fm1.errorCheckFlg.value != "0") {
		...
		if (errorCheckFlg == "1") {
			//ND103_医師勤務先追加
			document.fm1.paramDocNo.value	= document.fm1.docNo.value;
			document.fm1.screenId.value	= "ND103";
		    document.fm1.functionId.value = "Init"
		}
		...
		if (msgStr == "") {//アクションボタン押下後エラーがない場合

			//「勤務先削除」のエラーがない場合下記に遷移
			if (errorCheckFlg == "3") {
				//ND104_医師勤務先削除画面へ
				document.fm1.screenId.value	= "NF501";
			    document.fm1.functionId.value = "Init"
