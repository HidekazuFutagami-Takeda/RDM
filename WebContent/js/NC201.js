 ...
 * 組織選択ボタン押下
 */
function nC201SelectSos(bumonRank, sosCd,
 ...
 * Ajaxコール関数：組織・従業員検索
 */
function nC201CallAjax(searchUpSosCd, bumonRankPop, trtCd, batchCall) {
  ...
  ajax.setParameter("trtCd",g_trtCd);
...
function nc201SearchSosCd(){
	  // 2度押し対策
	  if(!comChkClickFlg(COM_CLICK_ALERT)){return false;}
	  // 選択フラグ「1：組織」
	  document.fm1.selectFlg.value	 = "1";

	  gCseViewWin = nc201CseView(gCseViewWin,"nc201CallBack","gCseViewWin");
	  return false;
}

function nc201CallBack(
		  bumonRank, sosCd,
		  bumonSeiName, brCode, distCode, upSosCode, upBumonRank, upBrCode, upDistCode){
...
function nc201CseView(w, callBack, winVarName, clearProc){
	  if(typeof(w)=='string'){
	    w=null;
	  }else{
	    //フォーカスをあてる処理と２度押し対策を共通関数化
