...
function cdcView(w,callBack,winVarName){
  ...
  comPostPopup(w,"JKR100C070Init.action",CDC_SCREEN_ID,COM_FUNC_VIEWINIT,callBack,winVarName,jokenName,joken,false);
...
function cdcCheckMaxSize(inputObject) {
  ...
  checkedList = cdcEncodeCodeData( checkedList );
  
  // チェック済みのコードを送信フォームにhidden属性でタグ追加する。
  document.getElementById("cdcCheckedCodesDiv").innerHTML = "";
  for (var i = 0; i < checkedList.length; i++) {
    document.getElementById("cdcCheckedCodesDiv").innerHTML += ""
