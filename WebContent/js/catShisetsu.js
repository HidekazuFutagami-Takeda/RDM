 ...
 * スクリーンID
 */
var CSE_SCREEN_ID = "CatShisetsu";

/**
 * タイトル(施設選択)
 */
var CSE_TITLE_SOS = "施設選択";
 ...
 * 施設を選択ボタン押下
 */
function cseSelectIns(insAbbrName,insFormalName,insNo,insAddr,shisetsuNmRyaku,shisetsuNm,dcfShisetsuCd,address,jgiName,insSbt,hoInsType) {
	if(!comChkSubmit(COM_CLICK_ALERT))return(false);
  eval("window.opener." + document.fm1.callBack.value + "(insAbbrName,insFormalName,insNo,insAddr,shisetsuNmRyaku,shisetsuNm,dcfShisetsuCd,address,jgiName,insSbt,hoInsType)"
  );
      cseClose();
}
