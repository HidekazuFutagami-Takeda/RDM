 ...
 */
@Named
public class NC011Service extends BaseService {
	 ...
	 * @generated
	 */
	private static Log log = LogFactory.getLog(NC011Service.class);
...
//         	indto.setReqYmdhmsFrom(indto.getReqYmdhmsFrom().substring(0, 4)+ '-' + indto.getReqYmdhmsFrom().substring(4, 6)+ '-' + indto.getReqYmdhmsFrom().substring(6, 8));
	...
	// ドロップダウンリスト-申請区分
	private void reqTypeDrop(NC011DTO indto) {
		// START UOC
		String check = "全て";
		int idFlg;
	...
	// ドロップダウンリスト-申請ステータス
	private void reqStsDrop(NC011DTO indto) {
		// START UOC
	...
	// ドロップダウンリスト-施設分類
	private void insClassDrop(NC011DTO indto) {
		// START UOC
	...
	// ドロップダウンリスト-施設種別
	private void insSbtDrop(NC011DTO indto) {
		// START UOC
	...
	// ドロップダウンリスト-医師種別
	private void docTypeDrop(NC011DTO indto) {
		// START UOC
	...
	// ドロップダウンリスト-勤務形態
	private void jobFormDrop(NC011DTO indto) {
		// START UOC
	...
	// ドロップダウンリスト-対象区分
	private void hoInsTypeDrop(NC011DTO indto) {
		// START UOC
...
//      }
	...
	// FB申請要否
	private void FbReqFlg(NC011DTO indto) {
		// START UOC
	...
	// FB申請要否
	private void FbPrcType(NC011DTO indto) {
		// START UOC
	...
	public BaseDTO search(NC011DTO indto) throws ParseException {
		...
		indto.initPageInfo(indto.getPageCntCur(), selectParamSelectReqList.get(0).getCntReq(),
				selectParamNC011List.get(1).getValue());
		...
		if (indto.getLineCntAll() > selectParamNC011List.get(0).getValue()) {
		...
		for (SRdmReqListEntity entiry : deptListEntity) {
			...
			if (entiry.getReqType() != null) {
				if (entiry.getReqType().equals("01") || entiry.getReqType().equals("02")
						|| entiry.getReqType().equals("03") || entiry.getReqType().equals("04")
						|| entiry.getReqType().equals("11") || entiry.getReqType().equals("12")
						|| entiry.getReqType().equals("13") || entiry.getReqType().equals("21")) {
					dataRecord.setSbt("施設");
				} else if (entiry.getReqType().equals("31") || entiry.getReqType().equals("32")
						|| entiry.getReqType().equals("33") || entiry.getReqType().equals("34")
						|| entiry.getReqType().equals("41") || entiry.getReqType().equals("42")
						|| entiry.getReqType().equals("43") || entiry.getReqType().equals("44")
						|| entiry.getReqType().equals("51")) {
			...
			if (entiry.getReqYmdhmsTo() != null) {
				if (entiry.getReqYmdhmsTo().length() == 8) {
					dataRecord.setReqYmdhmsTo(entiry.getReqYmdhmsTo().substring(0, 4) + '/'
							+ entiry.getReqYmdhmsTo().substring(4, 6) + '/' + entiry.getReqYmdhmsTo().substring(6, 8));
				} else if (entiry.getReqYmdhmsTo().length() == 12) {
					dataRecord.setReqYmdhmsTo(entiry.getReqYmdhmsTo().substring(0, 4) + '/'
							+ entiry.getReqYmdhmsTo().substring(4, 6) + '/' + entiry.getReqYmdhmsTo().substring(6, 8)
							+ ' ' + entiry.getReqYmdhmsTo().substring(8, 10) + ':'
							+ entiry.getReqYmdhmsTo().substring(10, 12));
				} else if (entiry.getReqYmdhmsTo().length() == 14) {
					dataRecord.setReqYmdhmsTo(entiry.getReqYmdhmsTo().substring(0, 4) + '/'
							+ entiry.getReqYmdhmsTo().substring(4, 6) + '/' + entiry.getReqYmdhmsTo().substring(6, 8)
							+ ' ' + entiry.getReqYmdhmsTo().substring(8, 10) + ':'
							+ entiry.getReqYmdhmsTo().substring(10, 12) + ':'
							+ entiry.getReqYmdhmsTo().substring(12, 14));
				}
				// dataRecord.setReqYmdhmsTo(entiry.getReqYmdhmsTo());
			} else {
				dataRecord.setReqYmdhmsTo(" ");
			}
			// dataRecord.setReqYmdhms(entiry.getReqYmdhms());
			// /*setDeptCode部分を修正している(DepjtK)*/
...
//                	}
			...
			if (entiry.getAPR_YMDHMS() != null) {
				if (entiry.getAPR_YMDHMS().length() == 8) {
					dataRecord.setAPR_YMDHMS(entiry.getAPR_YMDHMS().substring(0, 4) + '/'
							+ entiry.getAPR_YMDHMS().substring(4, 6) + '/' + entiry.getAPR_YMDHMS().substring(6, 8));
				} else if (entiry.getAPR_YMDHMS().length() == 12) {
					dataRecord.setAPR_YMDHMS(entiry.getAPR_YMDHMS().substring(0, 4) + '/'
							+ entiry.getAPR_YMDHMS().substring(4, 6) + '/' + entiry.getAPR_YMDHMS().substring(6, 8)
							+ ' ' + entiry.getAPR_YMDHMS().substring(8, 10) + ':'
							+ entiry.getAPR_YMDHMS().substring(10, 12));
				} else if (entiry.getAPR_YMDHMS().length() == 14) {
					dataRecord.setAPR_YMDHMS(entiry.getAPR_YMDHMS().substring(0, 4) + '/'
							+ entiry.getAPR_YMDHMS().substring(4, 6) + '/' + entiry.getAPR_YMDHMS().substring(6, 8)
							+ ' ' + entiry.getAPR_YMDHMS().substring(8, 10) + ':'
							+ entiry.getAPR_YMDHMS().substring(10, 12) + ':'
							+ entiry.getAPR_YMDHMS().substring(12, 14));
				}
			...
			if (entiry.getShnYmdhms() != null) {
				if (entiry.getShnYmdhms().length() == 8) {
					dataRecord.setShnYmdhms(entiry.getShnYmdhms().substring(0, 4) + '/'
							+ entiry.getShnYmdhms().substring(4, 6) + '/' + entiry.getShnYmdhms().substring(6, 8));
				} else if (entiry.getShnYmdhms().length() == 12) {
					dataRecord.setShnYmdhms(entiry.getShnYmdhms().substring(0, 4) + '/'
							+ entiry.getShnYmdhms().substring(4, 6) + '/' + entiry.getShnYmdhms().substring(6, 8) + ' '
							+ entiry.getShnYmdhms().substring(8, 10) + ':' + entiry.getShnYmdhms().substring(10, 12));
				} else if (entiry.getShnYmdhms().length() == 14) {
					dataRecord.setShnYmdhms(entiry.getShnYmdhms().substring(0, 4) + '/'
							+ entiry.getShnYmdhms().substring(4, 6) + '/' + entiry.getShnYmdhms().substring(6, 8) + ' '
							+ entiry.getShnYmdhms().substring(8, 10) + ':' + entiry.getShnYmdhms().substring(10, 12)
							+ ':' + entiry.getShnYmdhms().substring(12, 14));
				}
