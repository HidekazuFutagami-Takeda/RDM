 ...
 */
function postReturnRDM(addrNamePref,addrNameCity,addrNameArea,postCode,addrCodePref,addrCodeCity,tkPrefCd,tkCityCd,tkCityName,addrKanaPref,addrKanaCity,addrKanaArea){
	//１件選択パターンの場合は、渡したい値をそのままコールバック関数に渡すことにする。
	eval("window.opener." + document.fm1.callBack.value + "(addrNamePref,addrNameCity,addrNameArea,postCode,addrCodePref,addrCodeCity,tkPrefCd,tkCityCd,tkCityName,addrKanaPref,addrKanaCity,addrKanaArea)");
	dcaClose();

}
function dcaReturnRDM(deptCode,DeptKj,DeptKn){
	//１件選択パターンの場合は、渡したい値をそのままコールバック関数に渡すことにする。
	eval("window.opener." + document.fm1.callBack.value + "(deptCode,DeptKj,DeptKn)");
	dcaClose();
//dcaCloseがないと画面を閉じれない。
}
 ...
 */function dcaChkNumValue(str){
 ...
 function ctcCreateDeptCodeString() {
	...
	for(i = 0;i < rowCnt;i++){
		var submitName = "catDeptsComboDataList[" + i + "].submit";
		var DeptCode   = "catDeptsComboDataList[" + i + "].deptCode";

		if((!document.getElementsByName(submitName)[0].checked) && (document.getElementsByName(DeptCode)[0].value != "")){
			if (DeptCodeString == "") {
				DeptCodeString = "" + document.getElementsByName(DeptCode)[0].value;
			} else {
				DeptCodeString = DeptCodeString + "," + document.getElementsByName(DeptCode)[0].value;
			}
		}
	}
	return DeptCodeString;
}
 function ctcCreateDeptKjString() {
		...
		for(i = 0;i < rowCnt;i++){
			var submitName = "catDeptsComboDataList[" + i + "].submit";
			var DeptKj = "catDeptsComboDataList[" + i + "].deptKj";

			if((!document.getElementsByName(submitName)[0].checked) && (document.getElementsByName(DeptKj)[0].value != "")){
				if (DeptKjString == "") {
					DeptKjString = "" + document.getElementsByName(DeptKj)[0].value;
				} else {
					DeptKjString = DeptKjString + "," + document.getElementsByName(DeptKj)[0].value;
				}
			}
		}
		return DeptKjString;
	}
 function ctcCreateDeptKnString() {
		...
		for(i = 0;i < rowCnt;i++){
			var submitName = "catDeptsComboDataList[" + i + "].submit";
			var DeptKn = "catDeptsComboDataList[" + i + "].DeptKn";

			if((!document.getElementsByName(submitName)[0].checked) && (document.getElementsByName(DeptKn)[0].value != "")){
				if (DeptKnString == "") {
					DeptKnString = "" + document.getElementsByName(DeptKn)[0].value;
				} else {
					DeptKnString = DeptKnString + "," + document.getElementsByName(DeptKn)[0].value;
				}
			}
		}
		return DeptKnString;
